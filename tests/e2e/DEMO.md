...
| `temp_modify` | Adversarial scenario 3 (graduated to deterministic) | Two-phase staged-payload drop in the temp directory: a benign file rewritten into a malicious one | `file_system_tampering` |
| `nonsensitive_path` | Adversarial scenario 4 (graduated to deterministic) | Credentials staged outside the sensitive-path catalogue plus routine HTTPS egress | `sensitive_material_egress` |
| `pgserve_postinstall` | [pgserve 1.1.11-1.1.13 npm compromise](https://www.stepsecurity.io/blog/pgserve-compromised-on-npm-malicious-versions-harvest-credentials) (April 2026) | `postinstall` stealer reading npm / ssh / aws / cloud / wallet secrets and POSTing to an Internet Computer canister | `credential_harvest` |
| `agent_config_tamper` | IronWorm, SANDWORM_MODE and the Mitiga rogue-MCP-server shape | Writer running from the temp directory injecting hostile hooks, rules and MCP entries into the coding agent's own config (`~/.cursor/rules/*.mdc`, `.cursorrules`) | `file_system_tampering` |
| `agent_cred_harvest` | IronWorm (Jun 2026), Bitwarden CLI hijack (Apr 2026), Djinn Stealer, `codexui-android` | One process holding the agent's own credential stores (`~/.claude/.credentials.json`, `~/.codex/auth.json`, `~/.cursor/mcp.json`) plus SSH / AWS open while POSTing out | `credential_harvest` |
| `agent_denylist_bypass` | Ona report "How Claude Code escapes its own denylist and sandbox" | Transcript shows a permission-denied `curl` re-issued as `/usr/bin/curl` / `bash -c 'curl ...'` and succeeding | `agent_denylist_bypass` |
