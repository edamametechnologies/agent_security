...
def _enable_fast_cron(vm_name: str, out_path: Path) -> bool:
    ...
    inner_cmd = (
        ...
        "\n"
        "# Legacy edamame-extrapolator skill has been removed in favour of\n"
        "# the compiled extrapolator_run_cycle plugin tool plus EDAMAME's\n"
        "# host-side transcript observer; clean up any leftover alias copy\n"
        "# from older installs.\n"
        "for legacy_dir in (\n"
        "    os.path.expanduser('~/.openclaw/skills/edamame-extrapolator'),\n"
        "    os.path.expanduser('~/.openclaw/skills/edamame-cortex-extrapolator'),\n"
        "):\n"
        "    try:\n"
        "        if os.path.lexists(legacy_dir):\n"
        "            if os.path.islink(legacy_dir) or os.path.isfile(legacy_dir):\n"
        "                os.unlink(legacy_dir)\n"
        "            else:\n"
        "                shutil.rmtree(legacy_dir, ignore_errors=True)\n"
        "    except Exception as e:\n"
        "        print(f'WARNING: could not remove legacy skill dir {legacy_dir}: {e}')\n"
        "\n"
